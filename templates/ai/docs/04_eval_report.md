...
| 3 | Untagged / Mis-tagged Spend (RDS/EC2 thiếu tag `owner` hoặc `team`) | Happy | `tag-for-review` | `tag-for-review` (Confidence: 0.90) | PASS |
...
- **Procedure**:
  ...
  4. Lưu kết quả DynamoDB và so sánh nhãn dự đoán (Predicted) với nhãn thực tế (Ground Truth) để tính toán các chỉ số hiệu năng.
...
2. **Gap**: Phản hồi hai chiều từ Slack chưa tự động cập nhật lại baseline.
   → **Plan**: Xây dựng API feedback loop tự động ghi nhãn whitelist vào DynamoDB để tái huấn luyện mô hình ML hàng tuần.
