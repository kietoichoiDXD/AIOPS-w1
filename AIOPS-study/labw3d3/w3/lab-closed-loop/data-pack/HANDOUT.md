...
Record the results of running the 6 chaos scenarios (see section 5).

---

## 5. Acceptance: 6 chaos scenarios

After building, run these 6 tests in order. Paste the log output into `SUBMIT.md`.
...
```bash
# Terminal 1: run orchestrator with blast_radius.max_actions_per_minute: 2
uv run python closed_loop.py --config config.yaml

# Terminal 2: rapidly inject faults on 3 different services within 60 seconds
bash data-pack/scripts/inject_fault.sh latency payment-svc 500ms
sleep 5
...
# Expected:
# - First 2 alerts trigger actions successfully
# - 3rd alert detected but orchestrator logs BLAST_RADIUS_EXCEEDED
# - Action NOT executed on 3rd service
# - Blast-radius gauge decrements with each action, alert escalated when limit reached
# - Log: event_type=BLAST_RADIUS_EXCEEDED, action=escalate
```
...
# Terminal 2: inject cascading failure (api-gateway down → triggers errors in dependent services)
bash data-pack/scripts/inject_fault.sh kill api-gateway
sleep 30
# This causes HighErrorRate alerts on payment-svc and inventory-svc due to failed upstream calls

# Expected:
# - Orchestrator detects InstanceDown on api-gateway first
# - Action: restart api-gateway
# - Within 60s, downstream services (payment-svc, inventory-svc) fire HighErrorRate alerts
# - Orchestrator processes alerts in order, respects per-service mutex
# - After api-gateway recovers, downstream error rates return to normal
# - All verifications pass
# - Log sequence: ALERT_DETECTED (api-gateway) → ACTION_SUCCESS → ALERT_DETECTED (payment-svc) → ACTION_SUCCESS (or auto-resolves)
```

### Scenario 6 — Verify timeout and recovery

```bash
# Terminal 1: run orchestrator with verify_timeout_seconds: 30
uv run python closed_loop.py --config config.yaml

# Terminal 2: inject latency, then manually fix before verify completes
bash data-pack/scripts/inject_fault.sh latency payment-svc 500ms
...
# Expected:
# - Orchestrator detects HighLatency alert
# - Action executes (restart payment-svc)
# - Verify starts polling Prometheus
# - Service recovers during verify window (between poll samples)
# - After 3 consecutive passing samples: verify logs VERIFY_PASS
# - Log: event_type=VERIFY_PASS with sample_count=3, metric_value < threshold
# - Demonstrates verify polling strategy handles timing edge cases
```

---

## 6. Rubric (8 criteria, scale 1–5)
...
| 6 | **Blast-radius enforcement** | No rate limiting or incorrect implementation | Rate limiting exists but allows exceeding configured limits | Correct blast-radius enforcement: refuses actions when limit reached + logs BLAST_RADIUS_EXCEEDED + gauge metrics accurate |
| 7 | **Cascading failure handling** | Cannot handle multiple related alerts; processes alerts in random order | Processes alerts but may execute redundant actions on already-recovering services | Correctly handles cascade: prioritizes root cause (upstream service) + skips redundant actions + downstream auto-resolves after upstream fix |
| 8 | **Verify resilience** | Verify always uses single sample or has no timeout | Verify polls multiple times but fails on timing edge cases | Verify with configurable poll interval + min_samples requirement + handles service recovery mid-verify window + timeout protection |

**Scoring thresholds:**
- Passing level (scenarios 1–3 only): total ≥ 12/25 (criteria 1-5 only)
- Good level (scenarios 1–4): total ≥ 18/30 (criteria 1-6)
- Excellent level (scenarios 1–6): total ≥ 30/40 (all 8 criteria)
...
- You do **not** need to write unit tests for every function — the 6 chaos scenarios are the primary verification method.
