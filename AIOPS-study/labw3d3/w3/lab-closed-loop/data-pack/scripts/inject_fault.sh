...
case "$FAULT" in
  latency)
    ...
    DELAY_MS="${DELAY//ms/}"
    docker exec -u 0 "$CONTAINER" tc qdisc add dev eth0 root netem delay "${DELAY_MS}ms" 2>/dev/null \
      || docker exec -u 0 "$CONTAINER" tc qdisc change dev eth0 root netem delay "${DELAY_MS}ms"
    echo "[inject_fault] Latency ${DELAY} applied to $CONTAINER."
    echo "               To remove: docker exec -u 0 $CONTAINER tc qdisc del dev eth0 root"
    ;;
  ...
  clear-latency)
    ...
    docker exec -u 0 "$CONTAINER" tc qdisc del dev eth0 root 2>/dev/null || true
