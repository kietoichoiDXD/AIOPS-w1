...
Bài workshop đã được chạy và kiểm chứng trên đủ 3 phần:
...
## 2. Cách tôi dùng kiến thức của lab để giải bài

Tôi không giải bài theo kiểu chỉ chạy notebook cho ra kết quả, mà gắn từng phần với đúng nhóm kiến thức của workshop.
...
- `3σ` được dùng như baseline detector dễ giải thích
- `z-score` được dùng để đo mức lệch so với steady-state
...
- các pattern count lớn như TLS, DNS, timeout được dùng như evidence cho RCA
...
- trong `ex03`, quyết định không chỉ dựa trên detector mà còn dựa trên confidence và remediation safety
...
### Thuật toán 1 - Phát hiện anomaly metric
...
### Thuật toán 2 - Log template clustering
...
- `3σ` bám rất sát mốc alert thật, phù hợp làm baseline explainable detector
