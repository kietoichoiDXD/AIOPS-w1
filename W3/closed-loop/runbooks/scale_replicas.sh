...
while [[ "$CUR_DIR" != "/" && -n "$CUR_DIR" ]]; do
  if [[ -f "$CUR_DIR/configs/docker-compose.yml" ]]; then
    COMPOSE_FILE="$CUR_DIR/configs/docker-compose.yml"
    break
  fi
  if [[ "$CUR_DIR" =~ ^[A-Za-z]:/$ ]]; then
    break
  fi
...
if [[ -z "$COMPOSE_FILE" ]]; then
  COMPOSE_FILE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/../../configs/docker-compose.yml"
fi
