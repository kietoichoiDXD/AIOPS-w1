...
5. [Đặc tả API Endpoints](#5-đặc-tả-api-endpoints)
   - 5.1 `POST /v1/detect` — Phát hiện Bất thường (Đồng bộ — SLO P99 < 300ms)
   - 5.2 `POST /v1/decide` — Lập Kế hoạch Can thiệp
   - 5.3 `POST /v1/verify` — Xác thực Kết quả
   - 5.4 `GET /health` — Health Check
   - 5.5 `GET /v1/status/{anomaly_id}` — Kiểm tra Trạng thái Remediation
...
> [!IMPORTANT]
> **v3.2.0 (telemetry-contract.md):** Idempotency hot path dùng **DynamoDB conditional write + TTL 24h** — latency ~5–15ms, đáp ứng P99 `/v1/detect` < 300ms.
...
**Format bắt buộc (v1.5.0 — suffix theo bước pipeline):**

```
{tenant_id}:{billing_period_date}:{step}
```

| Bước | `step` suffix | Ví dụ |
|---|---|---|
| `/v1/detect` | `detect` | `a1b2c3d4-...:2026-06-23:detect` |
| `/v1/decide` | `decide` | `a1b2c3d4-...:2026-06-23:decide` |
| `/v1/verify` | `verify` | `a1b2c3d4-...:2026-06-24:verify` |

> [!WARNING]
> **Không dùng chung một key** (vd: `:daily-batch`) cho cả 3 endpoint trong cùng phiên — sẽ gây `400 ERR_IDEMPOTENCY_MISMATCH` vì request body khác nhau nhưng partition key trùng.
...
- CDO **chặn containment ở application code** (Lambda orchestrator không gọi boto3 execute) — **KHÔNG** tự động sửa IAM Policy, Permission Boundary hay Security Group ở runtime (vi phạm quy tắc cứng Capstone)
...
| `X-Idempotency-Key` | string | ✓ | Format: `{tenant_id}:{billing_period_date}:{step}` — `step` ∈ `detect` \| `decide` \| `verify` (xem §3.2) |
...
Nhận telemetry (CUR pointer hoặc RAW_JSON nhỏ), validate schema, chạy Isolation Forest scoring, trả `200 OK` + `DetectResponse` trực tiếp.

> [!IMPORTANT]
> **SLO cố định (không thương lượng):** P99 latency < **300ms** — bao gồm validate + idempotency + model scoring. **Không** gọi Bedrock trong `/v1/detect`.
>
> **Để đạt SLO với batch lớn:**
> - Daily batch **bắt buộc** `data_source_type = S3_POINTER` — CDO không POST embed full CUR.
> - AI Engine scoring trên **feature vector đã materialize** (DynamoDB `finops-feature-store-{env}`), không parse toàn bộ file `.json.gz` trong hot path.
> - `RAW_JSON` chỉ cho ad-hoc / payload ≤ 10MB.
...
| `data_confidence` | string (Enum) | ✓ | `HIGH` (CUR đầy đủ) hoặc `LOW` (CE fallback) |
| `error_message` | string | optional | Chi tiết lỗi khi `success = false` |

**Lược đồ Schema Phản hồi (`DetectResponse`)**:
...
```json
{
  ...
  "properties": {
    "correlation_id":  { "type": "string", "format": "uuid" },
    "idempotency_key": { "type": "string", "pattern": "^[a-f0-9-]{36}:[0-9]{4}-[0-9]{2}-[0-9]{2}:(detect|decide|verify)$" },
...
CDO sử dụng endpoint này để kiểm tra trạng thái **remediation** theo `anomaly_id` (format: `ANM-YYYY-MMDD[A-Z]`). **Không** dùng cho detect.

> [!NOTE]
> **v1.5.0 — Tách `audit_id` vs `anomaly_id` (đồng bộ telemetry-contract.md §16):**
> - `anomaly_id` (`ANM-...`) — định danh bất thường, dùng làm path param endpoint này
> - `audit_id` (UUID v4) — định danh bản ghi kiểm toán trong audit chain, trả trong response body
...
| Trường (Field) | Kiểu dữ liệu (Type) | Bắt buộc (Required) | Mô tả (Description) |
|---|---|---|---|
| `audit_id` | string (UUID v4) | ✓ | Định danh bản ghi kiểm toán — đồng bộ telemetry-contract §16 |
| `anomaly_id` | string | ✓ | Định danh bất thường (`ANM-YYYY-MMDD[A-Z]`) — khớp path param |
...
>
> 1. Khi `/v1/decide` trả về, CDO **lập tức cache `rollback_payload.boto3_equivalent` vào DynamoDB local** (table `finops-rollback-cache-{env}`, partition key `anomaly_id`).
...
|---|---|---|
| **P99 Latency** `/v1/detect` | **< 300 ms** (cố định) | Validate + idempotency + Isolation Forest scoring. **Không** Bedrock |
| **P99 Latency** `/v1/decide` — Compute path | < 500 ms | Validate + idempotency + runbook lookup. **Không** bao gồm Bedrock LLM |
| **P99 Latency** `/v1/decide` — LLM path | < 30 s · hard timeout 45 s | Bedrock Nova inference; > 10 s → fallback rule-based (xem §17.3 PromQL) |
...
sequenceDiagram
    ...
    EB->>CDO: Start Step Functions (Lambda trigger thoát ngay)
    CDO->>CDO: [SFN] Feature materialize từ S3 → DynamoDB (offline step)
    CDO->>AI: POST /v1/detect (S3_POINTER — body nhỏ)
    AI->>AI: Scoring trên feature store + Isolation Forest
    AI-->>CDO: 200 DetectResponse (anomalies_list, data_confidence)
...
| Tài liệu | Mô tả |
|---|---|
| `telemetry-contract.md` v3.2.0 | Schema CUR/CE/CloudWatch — CE conditional, cpu_utilization_hourly |
| `deployment-contract.md` v1.3.0 | ECS Fargate, IAM Boundaries, Rollback cache, Rate limit |
...
| `contracts_audit_report.md` | Audit gaps — 4/6 vá trong ai-api-contract v1.5.0 |

---

## 11. Lịch sử hợp nhất tài liệu

Tài liệu v1.5.0 = v1.4.0 (sync detect) + vá `contracts_audit_report.md`:

| Nguồn | Vai trò | Quyết định |
|---|---|---|
| `ai-api-contract.md` (v1.4.0) | Baseline sync, P99 < 300ms | **Giữ nguyên** — đây là **Sync**, không Async |
| `ai-api-contract-v1.2.0-patch.md` | Đề xuất async webhook | **Từ chối** |
| `contracts_audit_report.md` | 6 gap cross-contract | Vá §1/§2/§3/§6 trong file này; §4/§5 → telemetry + deployment |
| `ai-api-contract2.md` | v1.2.0 CDO-P1→P6 | Giữ decide/verify sync + rollback độc lập |

### Gap đã vá (`contracts_audit_report.md` → `ai-api-contract.md`)

| # | Gap (audit) | Trạng thái v1.5.0 | Section |
|---|---|---|---|
| 1 | Idempotency key trùng detect/decide/verify | ✓ | §3.2, §4 |
| 2 | LOCKED mode sửa IAM runtime | ✓ (CDO app-code) | §3.3 |
| 3 | `audit_id` vs `anomaly_id` conflict | ✓ | §5.5 |
| 4 | Feature Store S3 vs DynamoDB | Pending | `deployment-contract.md` + `telemetry-contract.md` |
| 5 | IAM thiếu `s3:PutObject` | Pending | `deployment-contract.md` §3.4 |
| 6 | `resource_tags_user_environment` required | ✓ | §5.1 schema |

### Gap đã vá (từ `api_contract_gaps.md` — v1.4.0)

| # | Gap | Trạng thái |
|---|---|---|
| 1–8 | CE schema, CUR fields, metrics, confidence, rollback, replay, rate limit | ✓ (giữ từ v1.4.0) |

### Sync vs Async — kết luận TF2

| Pattern | TF2 `/v1/detect` |
|---|---|
| **Sync** (v1.4.0 / v1.5.0) | ✓ `200 OK` + `DetectResponse`, P99 < 300ms |
| **Async** (v1.2.0-patch) | ✗ Từ chối — phá SLO hợp đồng |

---

*AI API Contract v1.5.0 — TF2 FinOps Watch — 2026-06-26*
